    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (data.email && data.password) {
            login({
                ...
                onCompleted(data) {
                    if (data) {
                        setAuthUser({ userId: data.login.id, email: data.login.email, role: data.login.role});
    ...
    if (user) {
        return (
            <main className="flex min-h-3/4 m-8 flex-col items-center justify-center p-8 font-poppins">
                <h1 className="font-bold text-center text-2xl mb-10 text-black">Bonjour, {user.email}!</h1>
                <a> Mes réservation </a>
                <button 
